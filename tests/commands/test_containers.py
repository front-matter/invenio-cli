...
def test_containerize_install(
    ...
    assert commands.docker_helper.execute_cli_command.mock_calls == expected_setup_calls
...
@patch("invenio_cli.commands.containers.run_cmd")
@patch("invenio_cli.commands.containers.DockerHelper")
def test_update_statics_and_assets(p_docker_helper, p_run_cmd, mock_cli_config):
    commands = ContainersCommands(mock_cli_config, p_docker_helper())
    ...
    expected_copy2_calls = [
        call(Path("project_dir/static/."), "/opt/invenio/var/instance/static/"),
        call(Path("project_dir/assets/."), "/opt/invenio/var/instance/assets/"),
    ]
...
def test_lock_python_dependencies(
    p_docker_helper, p_run_cmd, p_listdir, mock_cli_config
):
    commands = ContainersCommands(mock_cli_config, p_docker_helper())
...
@pytest.mark.skip()
@patch("invenio_cli.commands.containers.listdir", return_value=["Pipfile.lock"])
@patch("invenio_cli.commands.containers.run_cmd")
@patch("invenio_cli.commands.containers.DockerHelper")
...
@patch("invenio_cli.commands.containers.run_cmd")
@patch("invenio_cli.commands.containers.DockerHelper")
def test_demo(p_run_cmd, p_docker_helper, mock_cli_config):
    commands = ContainersCommands(mock_cli_config, p_docker_helper())
